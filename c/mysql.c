...
const char *g_host_name = "dbs.goapk.com";
//const char *g_user_name = "root";
const char *g_user_name = "azmk";
//const char *g_password = "root";
const char *g_password = "mkaz)#@)2012pwd";
//const char *g_db_name = "test";
const char *g_db_name = "newgomarket";
...
//    sprintf(sql, "INSERT INTO `test`(`name`) VALUES('testname')");
    sprintf(sql, "SELECT FROM `sj_soft` LIMIT 0,20");

    if (executesql(sql))
        print_mysql_error(NULL);

    if (executesql("SELECT * FROM `sj_soft_` LIMIT 0,20")) // 句末没有分号
        print_mysql_error(NULL);
